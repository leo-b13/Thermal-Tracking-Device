  ...
  int pos = 90;
  int xPos = 90;
  const int servoPin = 2;  //Pin for tracking servo 
  const int xServoPin = 4; //Pin for xServo (vents)
  ...
  //Buttons / Switches
  const int leftPin = 19; //Move xServo left pin
  const int rightPin = 18; //Move xServo right pin
  const int togglePin = 26; //Toggle switch pin for automatic/manual mode
  ...
  void TrackingFunc() {
    ...
    float tempCheck = ((totalTemp/64) - (minTemp/64));
    if (tempCheck < tempThreshold) {
      Serial.println("Searching...");
      Serial.print("check :");
      Serial.println(tempCheck);
      pos += searchStep;
      if (pos >= maxServoAngle || pos <= minServoAngle) {
        searchStep = -searchStep;
      }
      pos = constrain(pos, minServoAngle, maxServoAngle);
      track_servo.write(pos);
      Serial.print("Search pos: ");
      Serial.println(pos);
    ...
    Serial.println("Person found! Entering tracking :)");
